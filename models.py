...
class FCNify_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        score_d5 = self.d5(d1)
        score = score_d1[:,:,:rows, :cols] + score_d2[:,:,:rows, :cols] + score_d3[:,:,:rows, :cols] + score_d4[:,:,:rows, :cols] + score_d5[:,:,:rows, :cols]
...
class Resnet_coach_vae(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=64, deconv = False, drop_ratio = 0.5):
        self.inplanes = 64
        ...
        self.pred1 = nn.Conv2d(100, 1, kernel_size=1, bias=True)

        self.upsample2 = bilinear(scale_factor = 16, mode='nearest')
        self.sigmoid = nn.Sigmoid()
        self.features = None
    ...
    def forward(self, x, alpha = 1, rand = False, coach=True):
        self.features = None
        mu = None
        logvar = None
        if not coach:
            size_ = x.size()
            random = Variable(torch.rand(size_[0], 1, int(size_[2]/16), int(size_[3]/16) ).cuda())
            self.features = random

        else:
            self.features, mu, logvar = self.get_feature(x) 
        
        size_ = self.features.size()
        self.features = self.features.view(size_[0], size_[1], size_[2]*size_[3])
        p,_ = self.features.topk(k = int(size_[2]*size_[3]*self.drop_ratio), dim = 2)
        partitions = p[:,:, -1]
        partitions = partitions.unsqueeze(2).expand(size_[0], size_[1], size_[2]*size_[3])
        mask = self.sigmoid(alpha*(self.features - partitions))
